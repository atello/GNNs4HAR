...
class MHealthDataset(InMemoryDataset):
    def __init__(self, root, variant, transform=None, pre_transform=None, pre_filter=None, **kwargs):
        ...
        self.activity_labels = {k: v for k, v in enumerate(
            ['Standing', 'Sitting', 'Lying down', 'Walking', 'Climbing stairs', 'Waist bends forward', 'Arms up',
        ...
        if variant == "ensemble":
            if self.params.get("all_corr_matrices", False):
                ...
                self.activity = self.params.get("activity", 0)  # if no activity id is passed, it returns activity {0: Standing}
                data_dict = torch.load(f"{self.processed_dir}/mhealth_ensemble_bm_{self.activity}.pt")
    ...
    def processed_file_names(self):
        if self.variant == "ensemble":
            return [f"mhealth_ensemble_bm_{i}.pt" for i in range(self.NUM_ACTIVITIES)] + ["all_corr_matrices.pt"]
        else:
            return [f'mhealth_{self.variant}.pt']
    ...
    def process(self):
        ...
        data_list, _ = mhealth_preprocessing.load_preprocessed_data(raw_data_path=self.raw_dir,
        ...
        if self.variant == "ensemble":
            if isinstance(data_list, dict):
                ...
                for key in data_list.keys():
                    filename = f"{self.processed_dir}/mhealth_ensemble_bm_{key}.pt"
                    file = {
